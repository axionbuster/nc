...
import Control.Monad.Combinators hiding (skipMany, skipSome, (<|>))
...
_hexdigit =
  satisfyAscii isHexDigit
    <&> fromIntegral @_ @Integer . \c ->
      if
...
lit2large = adhoc "literal too large"
 ...
 where
  ...
  gated_go bw
    | bw > fromIntegral (maxBound :: Word8) =
        const $ lit2large "integer literal"
...
char_encpfx =
  $( switch
       [|
         case _ of
           "u8" -> charset encset_char8type
           "u" -> charset encset_char16type
           "U" -> charset encset_char32type
           "L" -> charset encset_wchartype
         |]
   )
 where
  charset p = view (penv_settings . setg_encoding . p) <$> ask
...
string_literal_val = do
  ...
  let enc c
        ...
        | otherwise = adhoc "bad character" here
